    ...
    try {
      ...
      if (error.name === 'QuotaExceededError') {
        ...
        try {
          ...
          console.log('✅ Successfully saved minimal state after emergency cleanup');
        } catch (retryError) {
    ...
    if (outboundState && typeof outboundState === 'object') {
      ...
      if (cleaned.favorites && Array.isArray(cleaned.favorites)) {
        ...
        if (cleaned.favorites.length > maxFavorites) {
          console.warn(`🚨 Too many favorites (${cleaned.favorites.length}), keeping only ${maxFavorites} most recent`);
...
const persistedAvatarReducer = persistReducer(avatarPersistConfig, avatarReducer);
