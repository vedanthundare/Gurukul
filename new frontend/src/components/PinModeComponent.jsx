...
const PinAvatarCard = memo(
  ({
    ...
    const avatarPosition = useMemo(() => {
      ...
      let baseOffset = -0.8; // Better offset for pin mode cards (fallback.glb)
    ...
    const avatarScale = useMemo(() => {
      ...
      let defaultScale = 3.0; // Default for fallback.glb
      if (favorite.previewUrl && favorite.previewUrl.includes('jupiter.glb')) {
    ...
    const avatarPath = useMemo(() => {
      ...
      if (favorite.isDefault) {
        // For default avatars, use their specific previewUrl (fallback.glb or jupiter.glb)
        return favorite.previewUrl || "/avatar/fallback.glb";
      } else if (favorite.isCustomModel) {
        return (
          favorite.previewUrl || favorite.fileData || "/avatar/fallback.glb"
        );
      } else {
        return (
          favorite.previewUrl || favorite.fileData || "/avatar/fallback.glb"
