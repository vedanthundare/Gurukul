...
const AvatarModel = memo(function AvatarModel({
  ...
  // Special handling for default avatars (fallback.glb and jupiter.glb)
  const isDefaultAvatar = useMemo(() =>
    url.includes("fallback.glb") || url.includes("jupiter.glb"), [url]);
  ...
  const {
    ...
    debug: url.includes("fallback.glb"), // Enable debug for fallback avatar
    enableProceduralAnimations: true,
  });

  // Set a timeout for loading - longer timeout for large files like fallback.glb
  useEffect(() => {
    const timeoutDuration = url.includes("fallback.glb") ? 15000 : 5000; // 15 seconds for fallback.glb
      ...
      >
        ...
        >
          ...
          <Suspense
            fallback={
              <AvatarLoading
                isLargeFile={currentPath?.includes("fallback.glb")}
              />
            }
          >
...
useGLTF.preload("/avatar/fallback.glb");
