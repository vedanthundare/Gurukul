...
export const orchestrationApiSlice = createApi({
  ...
  endpoints: (builder) => ({
      ...
      async onQueryStarted(arg, { queryFulfilled }) {
        try {
          ...
          if (data && (data.explanation || data.text || data.content)) {
            const contentToSpeak = data.explanation || data.text || data.content;

            if (contentToSpeak && contentToSpeak.trim()) {
              console.log("🔊 Orchestration TTS: Triggering auto-play for enhanced lesson");
              ...
              setTimeout(() => {
                ttsService.autoPlayAI(formattedContent, {
                  delay: 1000,
                  volume: 0.8
                }).catch(error => {
                  console.warn("🔊 Orchestration TTS: Auto-play failed:", error.message);
                });
              }, 500);
            }
          }
        } catch (error) {
          console.warn("🔊 Orchestration TTS: Failed to process lesson for TTS:", error);
        }
      },
    ...
    getUserProgress: builder.query({
      query: (user_id) => {
        console.log("📊 Fetching user progress for:", user_id);
        return `/user-progress/${user_id}`;
      },
    ...
    getUserAnalytics: builder.query({
      query: (user_id) => {
        console.log("📈 Fetching user analytics for:", user_id);
        return `/user-analytics/${user_id}`;
      },
      providesTags: (result, error, user_id) => [
        { type: "UserAnalytics", id: user_id }
      ],
    ...
    triggerIntervention: builder.mutation({
      query: ({ user_id, quiz_score = null }) => {
        console.log("🚨 Triggering intervention for user:", user_id, "with quiz score:", quiz_score);
        ...
        return {
          url: `/trigger-intervention/${user_id}${params.toString() ? `?${params.toString()}` : ''}`,
          method: "POST",
        };
      },
...
export const checkOrchestrationAvailability = async (dispatch) => {
  try {
    ...
    if (result.data) {
      const isAvailable = result.data.integration_status?.overall_valid && 
...
export const formatEnhancedLessonData = (lessonData) => {
  ...
  const isEnhanced = lessonData.source?.includes('orchestration') || 
...
export const formatUserProgressData = (progressData) => {
  ...
  return {
    ...
    averageScore: educationalProgress.quiz_scores?.length > 0 
      ? educationalProgress.quiz_scores.reduce((a, b) => a + b, 0) / educationalProgress.quiz_scores.length 
      : null,
    ...
    performanceStatus: educationalProgress.quiz_scores?.length > 0
      ? (educationalProgress.quiz_scores.slice(-3).every(score => score < 60) ? 'struggling' : 'good')
      : 'unknown'
  };
};
